...
export class SwalDataManager implements SwalManager {
  ...
  showCanceledAction(item: string): void {
    swalWithBootstrapButtons.fire('¡Acción cancelada!', `${item} seguro.`, 'error');
  }

  async requestConfirmationDeleteDocument(successfulAction: () => void): Promise<any> {
    return swalWithBootstrapButtons
      .fire({
  ...
  async requestConfirmationDeleteUserAsset(successfulAction: () => void): Promise<any> {
    return swalWithBootstrapButtons
      .fire({
